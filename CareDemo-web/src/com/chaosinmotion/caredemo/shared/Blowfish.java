...
/**
 * We encrypt all the packets that are sent back and forth between the client
 * and the server, despite the fact that we also use SSL/TLS in order to
 * encode the packets. See my blog post at http://chaosinmotion.com/blog/?p=967
 * @author woody
 *
 */
public class Blowfish implements Serializable
{
	...
	/**
	 * Encrypt a string. This encrypts the string as a block of 8-byte
	 * words with a header of 4 bytes with the length, encoded as UTF-8.
	 * @throws IOException 
	 */
	...
	public byte[] encrypt(String str)
	{
		...
		int blen = (len + 4 + 7) & ~7;	// length that fits, 8 byte boundary
		byte[] buffer = new byte[blen];
		buffer[0] = (byte)(len >> 24);
		buffer[1] = (byte)(len >> 16);
		buffer[2] = (byte)(len >> 8);
		buffer[3] = (byte)(len);
		System.arraycopy(strData, 0, buffer, 4, len);
	...
	public String decrypt(byte[] buffer)
	{
		...
		int len = 0x00FF & tmp[0];
		len = (len << 8) | (0x00FF & tmp[1]);
		len = (len << 8) | (0x00FF & tmp[2]);
		len = (len << 8) | (0x00FF & tmp[3]);
		
		try {
			return new String(tmp,4,len,"UTF-8");
		}
		catch (UnsupportedEncodingException e) {
			// Should never happen
			return null;
